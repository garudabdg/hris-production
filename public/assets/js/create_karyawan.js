...
async function loadModels() {
    ...
    try {
        await faceapi.nets.tinyFaceDetector.loadFromUri(window.CreateKaryawanConfig.modelsUrl);
...
async function captureFrame() {
    ...
    // Draw video to canvas (do NOT flip horizontally so the saved reference image matches the un-mirrored live camera)
...
async function finishScanning() {
    ...
    try {
        ...
        formData.append('_token', window.CreateKaryawanConfig.csrfToken);
        formData.append('nik', window.CreateKaryawanConfig.nik);
        ...
        $.ajax({
            type: 'POST',
            url: window.CreateKaryawanConfig.storeRoute,
            data: formData,
            processData: false,
            contentType: false,
...
function showSuccessScreen() {
    ...
    setTimeout(() => {
        window.location.href = window.CreateKaryawanConfig.dashboardRoute;
    }, 2000);
}
