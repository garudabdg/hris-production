...
@section('content')
    <div class="row">
        ...
        @if(($generalsetting->provider_wa ?? '') === 'local')
        {{-- ═══════════════════════════════════════════════════════════ --}}
        {{-- Panel LOCAL GATEWAY (Mandiri / Baileys)                    --}}
        {{-- ═══════════════════════════════════════════════════════════ --}}
        <div class="row mb-4">
            <div class="col-md-5">
                <div class="card h-100">
                    <div class="card-header bg-success text-white d-flex align-items-center justify-content-between">
                        <h5 class="card-title mb-0"><i class="ti ti-brand-whatsapp me-2"></i>WA Gateway DIDIMAX</h5>
                        <span class="badge bg-white text-success fw-semibold" id="local-badge">Memuat...</span>
                    </div>
                    <div class="card-body text-center">
                        {{-- Status Connected --}}
                        <div id="local-connected-panel" class="d-none">
                            <div class="mb-3">
                                <i class="ti ti-circle-check text-success" style="font-size:4rem"></i>
                            </div>
                            <h5 class="text-success">WhatsApp Terhubung</h5>
                            <p class="mb-1">Nomor aktif:</p>
                            <h4 class="fw-bold text-dark" id="local-connected-number">-</h4>
                            <hr>
                            <button class="btn btn-outline-danger btn-sm mt-2" id="btn-local-disconnect">
                                <i class="ti ti-plug-x me-1"></i>Disconnect
                            </button>
                        </div>
                        {{-- QR Panel --}}
                        <div id="local-qr-panel">
                            <div id="qr-loading" class="py-4">
                                <div class="spinner-border text-success" role="status"></div>
                                <p class="mt-2 text-muted">Memuat QR Code...</p>
                            </div>
                            <div id="qr-display" class="d-none">
                                <p class="text-muted small mb-2">Scan QR dengan WhatsApp di HP Anda:</p>
                                <img id="qr-image" src="" alt="QR Code" class="img-fluid rounded border" style="max-width:260px">
                                <p class="text-muted small mt-2">QR diperbarui otomatis setiap 30 detik</p>
                            </div>
                            <div id="qr-unavailable" class="d-none py-3">
                                <i class="ti ti-alert-circle text-warning" style="font-size:3rem"></i>
                                <p class="mt-2 text-muted" id="qr-unavailable-msg">QR belum tersedia</p>
                                <button class="btn btn-success btn-sm" id="btn-refresh-qr">
            ...
            <div class="col-md-7">
                <div class="card h-100">
                    <div class="card-header">
                        <h5 class="card-title mb-0">Cara Menghubungkan WhatsApp</h5>
                    </div>
                    <div class="card-body">
                        <ol class="mb-3">
                            ...
                            <li>Tunggu hingga status berubah menjadi <span class="badge bg-success">Terhubung</span></li>
                        </ol>
                        <div class="alert alert-info small mb-3">
                            <i class="ti ti-info-circle me-1"></i>
                            Session tersimpan otomatis. Setelah scan sekali, tidak perlu scan ulang meski server restart.
                        </div>
                        <div class="row g-2">
                            <div class="col-6">
                                <div class="card bg-light">
                                    <div class="card-body text-center py-2">
                                        <div class="fw-semibold small text-muted">Status Koneksi</div>
                                        <div id="local-conn-text" class="fw-bold mt-1">-</div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="card bg-light">
                                    <div class="card-body text-center py-2">
                                        <div class="fw-semibold small text-muted">Uptime Service</div>
                                        <div id="local-uptime-text" class="fw-bold mt-1">-</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        {{-- Test kirim pesan --}}
                        <div class="mt-3">
                            <h6 class="fw-semibold">Test Kirim Pesan</h6>
                            ...
                            <div class="input-group mb-2">
                                ...
                                <input type="text" class="form-control" id="test-message" placeholder="Pesan test..." value="Halo! Ini pesan test dari HRIS DIDIMAX.">
                            </div>
                            <button class="btn btn-success btn-sm w-100" id="btn-test-send">
                                <i class="ti ti-send me-1"></i>Kirim Test
                            </button>
        ...
        @else
        {{-- ═══════════════════════════════════════════════════════════ --}}
        {{-- Panel Third-Party Gateway (Portax/Fonnte/etc)              --}}
        {{-- ═══════════════════════════════════════════════════════════ --}}
        @endif
    ...
    <script>
        ...
        (function () {
            ...
            function updateStatus() {
                $.get('{{ route("wagateway.local.status") }}', function (data) {
                    const badge = $('#local-badge');
                    const connText = $('#local-conn-text');
                    if (data.connected) {
                        badge.attr('class', 'badge bg-white text-success fw-semibold').text('Terhubung ✓');
                        connText.html('<span class="text-success">Terhubung</span>');
                        $('#local-connected-number').text('+' + (data.number || '-'));
                        $('#local-connected-panel').removeClass('d-none');
                        $('#local-qr-panel').addClass('d-none');
                        if (qrInterval) { clearInterval(qrInterval); qrInterval = null; }
                    } else {
                        badge.attr('class', 'badge bg-white text-warning fw-semibold').text('Menunggu Scan');
                        connText.html('<span class="text-warning">Menunggu Scan</span>');
                        $('#local-connected-panel').addClass('d-none');
                        $('#local-qr-panel').removeClass('d-none');
                        if (!qrInterval) { loadQr(); qrInterval = setInterval(loadQr, 30000); }
                    }
                }).fail(function () {
                    $('#local-badge').attr('class', 'badge bg-white text-danger fw-semibold').text('Service Offline');
                    $('#local-conn-text').html('<span class="text-danger">Offline</span>');
                });

                // Uptime dari health
                $.get('{{ url("/") }}'.replace(/\/$/, '') + '/../../../..',  function(){}).always(function(){});
                fetch('/wa-gateway-health').catch(() => {});
            }

            function loadQr() {
                $('#qr-loading').removeClass('d-none');
                $('#qr-display').addClass('d-none');
                $('#qr-unavailable').addClass('d-none');
                $.get('{{ route("wagateway.local.qr") }}', function (data) {
                    $('#qr-loading').addClass('d-none');
                    if (data.connected) {
                        updateStatus();
                    } else if (data.qr) {
                        $('#qr-image').attr('src', data.qr);
                        $('#qr-display').removeClass('d-none');
                    } else {
                        $('#qr-unavailable-msg').text(data.msg || 'QR belum tersedia');
                        $('#qr-unavailable').removeClass('d-none');
                    }
                }).fail(function () {
                    $('#qr-loading').addClass('d-none');
                    $('#qr-unavailable-msg').text('Gagal mengambil QR – service mungkin offline');
                    $('#qr-unavailable').removeClass('d-none');
                });
            }

            // Tombol disconnect
            $('#btn-local-disconnect').on('click', function () {
                if (!confirm('Yakin ingin disconnect WhatsApp?')) return;
                $(this).prop('disabled', true).html('<span class="spinner-border spinner-border-sm"></span>');
                $.ajax({
                    url: '{{ route("wagateway.local.disconnect") }}',
                    type: 'POST',
                    headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                    success: function () {
                        Swal.fire('Berhasil', 'WhatsApp berhasil disconnect', 'success');
                        updateStatus();
                    },
                    error: function () {
                        Swal.fire('Gagal', 'Tidak bisa disconnect', 'error');
                    },
                    complete: function () {
                        $('#btn-local-disconnect').prop('disabled', false).html('<i class="ti ti-plug-x me-1"></i>Disconnect');
                    }
                });
            });

            // Tombol refresh QR
            $('#btn-refresh-qr').on('click', loadQr);

            // Test kirim pesan
            $('#btn-test-send').on('click', function () {
                const number  = $('#test-number').val().trim();
                const message = $('#test-message').val().trim();
                if (!number || !message) { alert('Isi nomor dan pesan!'); return; }

                $(this).prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Mengirim...');
                ...
                $.ajax({
                    ...
                    headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                    data: JSON.stringify({ sender: 'local', number: number, message: message }),
                    success: function (res) {
                        $('#test-result').removeClass('d-none').html(
                            '<div class="alert alert-success small py-2 mb-0"><i class="ti ti-check me-1"></i>' + (res.message || 'Pesan terkirim!') + '</div>'
                        );
                    },
                    error: function (xhr) {
                        const msg = xhr.responseJSON?.message || xhr.responseJSON?.debug?.response_body || 'Gagal kirim pesan';
                        $('#test-result').removeClass('d-none').html(
                            '<div class="alert alert-danger small py-2 mb-0"><i class="ti ti-x me-1"></i>' + msg + '</div>'
                        );
                    },
                    complete: function () {
                        $('#btn-test-send').prop('disabled', false).html('<i class="ti ti-send me-1"></i>Kirim Test');
                    }
                });
            });
