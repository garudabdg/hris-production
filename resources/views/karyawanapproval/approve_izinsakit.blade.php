...
@section('content')
    ...
    <div id="header-section">
        <div class="appHeader bg-primary text-light">
            ...
            <div class="pageTitle">Proses Izin Sakit</div>
            <div class="right"></div>
        </div>
    </div>

    <div id="content-section">
        <form action="{{ route('karyawan-approval.izinsakit.storeapprove', Crypt::encrypt($izinsakit->kode_izin_sakit)) }}" method="POST" id="frmApprove">
            @csrf
            <div class="info-card">
                <div class="info-header">
                    <div class="avatar">{{ strtoupper(substr($izinsakit->nama_karyawan, 0, 1)) }}</div>
                    <div class="info">
                        <div class="nama">{{ $izinsakit->nama_karyawan }}</div>
                        <div class="meta">{{ $izinsakit->nik }} · {{ $izinsakit->nama_jabatan }}</div>
                    </div>
                </div>
                <div class="detail-row">
                    <span class="lbl">Departemen</span>
                    <span class="val">{{ $izinsakit->nama_dept }}</span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Cabang</span>
                    <span class="val">{{ $izinsakit->nama_cabang }}</span>
                </div>
                <div class="detail-row">
                    <span class="lbl">Periode</span>
                    <span class="val">
                        {{ date('d/m/Y', strtotime($izinsakit->dari)) }} - {{ date('d/m/Y', strtotime($izinsakit->sampai)) }}
                        <br><span class="dur-badge">@php $lama = hitungHari($izinsakit->dari, $izinsakit->sampai); @endphp {{ $lama }} Hari</span>
                    </span>
                </div>
                <div class="detail-row">
