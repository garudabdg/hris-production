@extends('layouts.app')
@section('titlepage', 'Izin Keluar / Offline')

@section('content')
@section('navigasi')
    <span>Izin Keluar / Offline</span>
@endsection
<div class="row">
                            ...
                            <form action="{{ route('izinkeluar.index') }}">
                                <div class="row g-2">
                                    <div class="col-lg-6 col-md-12 col-sm-12">
                                        <x-input-with-icon label="Dari Tanggal" value="{{ Request('dari') }}" name="dari" icon="ti ti-calendar"
                                            datepicker="flatpickr-date" hideLabel />
                                    </div>
                                    <div class="col-lg-6 col-md-12 col-sm-12">
                                        <x-input-with-icon label="Sampai Tanggal" value="{{ Request('sampai') }}" name="sampai" icon="ti ti-calendar"
                                            datepicker="flatpickr-date" hideLabel />
                                    </div>
                                </div>
                                <div class="row g-2">
                                        ...
                                        <div class="row align-items-center">
                                            ...
                                            <div class="col-md-3 border-start border-end d-none d-md-block text-center">
                                                 ...
                                                 <div class="text-muted" style="font-size: 11px;">
                                                    {{ $d->kode_izin_keluar }} <span class="mx-1">•</span> Jam: {{ date('H:i', strtotime($d->jam_keluar)) }} - {{ $d->jam_kembali ? date('H:i', strtotime($d->jam_kembali)) : 'Selesai' }}
                                                 </div>
                                            </div>
                                                    ...
                                                    @if ($d->status == 0)
                                                        ...
                                                        @if($canCancel)
                                                            <form method="POST" name="deleteform" class="deleteform d-inline"
                                                                action="{{ route('izinkeluar.cancelapprove', Crypt::encrypt($d->kode_izin_keluar)) }}">
                                                                @csrf
                                                                @method('DELETE')
                                                ...
                                                @can('izinkeluar.edit')
                                                    @if ($d->status == 0)
                                                        <a href="#" class="btn btn-sm btn-outline-success btnEdit py-1 px-2 rounded-0"
                                                            kode_izin_keluar="{{ Crypt::encrypt($d->kode_izin_keluar) }}"><i
                            ...
                            @empty
                                <div class="card">
                                    <div class="card-body text-center p-5">
                                        ...
                                        <p class="text-secondary">Data izin keluar belum tersedia untuk periode atau filter yang dipilih.</p>
                                    </div>
                                </div>
                            @endforelse
...
<script>
    $(function() {
        ...
        $("#btnCreate").click(function() {
            $("#modal").modal("show");
            loading();
            $("#modal").find(".modal-title").text("Buat Izin Keluar / Offline");
